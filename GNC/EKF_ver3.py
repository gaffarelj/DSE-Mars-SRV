...
sig = [3/20*10**-3 for r in r_l]+[1/90*10**-3 for v in v_l]+[(3.5e-6)*9.81 for a in av]
h_val = 1e-3

dim = len(xv+vv+av)
r_l += v_l+ av

r = EKF.vector_func([smp.sqrt(np.dot((xv-p),(xv-p))) for p in p_l] + [np.dot((vv),(xv-p))/smp.sqrt(np.dot((xv-p),(xv-p))) for p in p_l]+[a for a in av],xv+vv+av)
r.lamb()
print(r.eval(x0))
