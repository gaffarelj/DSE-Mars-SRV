...
def Class_I_spaceplane_est(DeltaV, Ve, m_tot, TWR, m_wing):
    ...
    m_prop = m_frac * m_tot / (1 + m_frac)
    m_RCS = 0.0126 * m_tot
    Fvac = TWR * 3.7 * m_tot * 0.10 #Assumption: L/D of 10
    m_eng = 0.00514 * Fvac ** 0.92068
    m_tank = 0.15 * m_prop
    m_thr_str = 1.949 * 10 ** (-3) * (Fvac / 4.448) ** 1.0687 * 0.453
...
def takeoff_wing_sizing_shuttle_like(M_takeoff,takeoff_mach,takeoff_cl):
    ...
    b = 30.5/874.5 * S #Taken from the wing planform of the spaceshuttle, and scaled accordingly. Also, full span, not 1 wing
    taper = 0.2 #Roughly, from spaceshuttle planform again
    c_root = 0.5*0.5*b/(S*1.2)
    c_tip = taper * c_root
...
def AVID_wing_mass( M_land , b , S_exp , c_root , tc ):
    M_land = M_land * 2.205
    b = b / 0.3048
    S_exp = S_exp / (0.3048**2) * 0.9 #90% of wing exposed
    c_root = c_root / 0.3048
    Mwing = 1575*((M_land*3.75*b*S_exp)/(c_root*tc*10**9))**0.67
    Mwing = Mwing * 0.4536
    print(Mwing)
    Mwing = 1.498*S_exp/(0.3048**2)**1.176
    Mwing = Mwing * 0.4536
    print(Mwing)
    print(" ")
    return Mwing
